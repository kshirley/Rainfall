...
# Meta-parameters:
T <- 6779  # total number of days
S <- 6  # locations
J <- c(2,2,2,2,2,5) # rainfall time series per location
P <- 5
...
mu <- c(-5,-0.6,5,-4,-2)  # for intercept, linear, sin1, sin2, cos1, cos2
sigma <- c(1,0.5,0.5,0.5,0.5)
...
X <- cbind(rep(1,T),sin.mat[,1:2],cos.mat[,1:2])
P <- dim(X)[2]

# Simulate Z given X_t, beta, and V (lambda and tau)
Z <- mvrnorm(n=T,mu=rep(0,S),Sigma=V)
xbeta <- X %*% beta
Z <- Z + xbeta
...
W <- as.list(rep(NA,S))
gamma.mat <- matrix(NA,T,S)
for (s in 1:S){
  #print(s)
  gamma <- rgamma(n=T,shape=alpha/2,scale=2/alpha)
  W[[s]] <- mvrnorm(n=T,mu=rep(0,J[s]),Sigma=Sigma[[s]])/sqrt(gamma)
  #X.w <- cbind(rep(1,J[s]),X.arc[[s]])
  W[[s]] <- t(W[[s]] + matrix(Z[,s],T,J[s]) + matrix(rep(X.arc[[s]]*beta.arc[s],each=T),ncol=J[s]))
  gamma.mat[,s] <- gamma
}

# Truncate at zero, and delete values to match missing data properties of the real data:
R.complete <- as.list(rep(NA,S))  # with no missing data, but truncated
R <- as.list(rep(NA,S))  # with some data missing
for (s in 1:S){
  R.complete[[s]] <- W[[s]]
  R.complete[[s]][R.complete[[s]]<0] <- 0
  R[[s]] <- R.complete[[s]]
  R[[s]][na.mat[[s]]==1] <- NA
}
# Great. Now R is like our real data



#####KV---I think you end here if you're simulating data#####################################################
# save the true parameter values in a list:
true.values <- list(mu=mu,sigma=sigma,alpha=alpha,lambda=lambda,tau=tau,mu.arc=mu.arc,tau.arc=tau.arc,beta.arc=beta.arc,
                    beta=beta,Sigma=Sigma,Z=Z,W=W,gamma.mat=gamma.mat)
...
# Basic parameters: (degrees freedom and spatial dependence)
alpha.start <- c(10,10,10)
lambda.start <- c(1,3,5)

# ARC bias:
mu.arc.start <- c(-1,0,1)
tau.arc.start <- c(1,3,5)
beta.arc.start <- matrix(NA,K,S)
for (k in 1:K) beta.arc.start[k,] <- rnorm(S,mu.arc.start[k],tau.arc.start[k])

# Measurement error covariance
Sigma.start <- as.list(rep(NA,S))
for (s in 1:S) Sigma.start[[s]] <- diag(J[s])

# Do a regression of positive rainfall values on X to get rough starting points:
y.vec <- NA
X.mat <- matrix(NA,1,P)
for (s in 1:S){
  for (j in 1:J[s]){
    sel <- R[[s]][j,]>0 & !is.na(R[[s]][j,])
    y.vec <- c(y.vec,R[[s]][j,sel])
    X.mat <- rbind(X.mat,X[sel,])
  }
}
y.vec <- y.vec[-1]
X.mat <- X.mat[-1,]

fit <- lm(y.vec ~ 0 + X.mat)
plot(1:(2*365),X[1:(2*365),]%*%coef(fit))
# eh, looks OK, even though it never goes negative...

# Set starting points:
mu.start <- matrix(coef(fit),K,P,byrow=TRUE)
tau.start <- summary(fit)$sigma + c(-3,0,3)
sigma.start <- matrix(0.5,K,P)
beta.start <- array(NA,dim=c(K,P,S))
for (k in 1:K){
  for (p in 1:P){
    beta.start[k,p,] <- rnorm(S,mu.start[k,p],sigma.start[k,p])
  }
}
...
for (k in 1:K){
  ...
  Sigma <- as.list(rep(NA,S))
  #KV- [[s]] represents the list [k,,] represents row=1, matrix=1, column=1
  #KV-There is not kth dimension to Sigma.start in simulated data, so removing: [k,,]
  for (s in 1:S) Sigma[[s]] <- Sigma.start[[s]]
...
save(gibbs.list,file=paste(path,"gibbs_out_12232013_G5000_sim.RData",sep=""))


load(file=paste(path,"gibbs_out_12232013_G5000_sim.RData",sep=""))
...
# beta
# KV-There's no X.names in simulated data
...
# mu
# KV- X.names not in simulated data
...
#KV -Error in mu.gibbs[, 2501:G, i + 11] : subscript out of bounds
...
#K- Error in plot.xy(xy.coords(x, y), type = type, ...) : object 'col.blend' not found
...
#KV- Error in X[, 1:11] : subscript out of bounds
...
# Plot these posterior predictive checks:
pb <- seq(3000,5000,by=5)
#KV-I change the title based on which g.out I'm using
pdf(file=paste(path,"fig_tobit_ppred.pdf",sep=""),width=10,height=8)
par(mfrow=c(3,4))
for (i in 1:J.sum){
  for (m in 1:12){
