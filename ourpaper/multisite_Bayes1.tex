...
The explanation of the model is as follows. The first level of the model is a standard tobit regression, where we model the observed $Y_{stj}$ as being equal to the j$^{th}$ component of the latent multivariate $t$ random variable, $W_{st}$ if it is greater than zero, and equal to zero if the latent multivariate $t$ random variable is less than or equal to zero. Next, for each location $s$, the length-$J_s$ vector of latent rainfall amounts is a multivariate $t$ random variable centered at the mean rainfall amount for that location, $Z_{st}$, and offset by an ARC bias effect, $\beta_s^\text{ARC}$. $\bW_{st}$ is a multivariate $t$ random variable because it is a mixture of multivariate normals with a mixture weight, $\frac{1}{\gamma_{st}}$ that has a gamma distribution as its prior.
