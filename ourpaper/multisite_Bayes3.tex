...
0  &\mbox{if } W_{stj} \leq 0, \end{cases} \\
% Latent continuous rainfall
\quad \bW_{st} &\sim \text{N}_{J_s}(\mathbf{1}Z_{st} + \bX^\text{ARC}_s \beta^\text{ARC}_s, \frac{1}{\gamma_{st}}\Sigma_s), \\
% Underlying spatial mean rainfall process for each location:
...
% the variables for which we have no conjugate priors, namely $Z_t$ and [ANY OTHERS?]. 
