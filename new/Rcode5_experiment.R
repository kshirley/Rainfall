# Here, we'll load the full, observed data
# Then, we'll set to NA the last 4 years of Hager Selam station data
# Adjust the necesssary objects (Y, na.mat, etc.)

# Eventually we will fit 4 different models to this data
# First, let's focus on our full model
# One key difference is to save the W samples from the holdout data
...
# First, remove the last 4 years of hagereselam:
#data.frame(date.string, Y[[1]][2, ])  # look at the dates and values

# define the indices of the days to hold out for testing:
holdout.vector <- which(date.string == "2004-01-01"):which(date.string == "2007-12-31")

# actually replace the values and the NA indicators with "NA" and "1" respectively:
Y[[1]][2, holdout.vector] <- NA
na.mat[[1]][2, holdout.vector] <- 1


# Proceed with the MCMC:
...
# Save samples of W for the holdout data:
#W.gibbs <- as.list(rep(NA, S))
#for (s in 1:S) W.gibbs[[s]] <- array(NA, dim=c(K, G, J[s], n.samp))
Wholdout.gibbs <- array(NA, dim = c(K, G, length(holdout.vector)))
...
for (k in 1:K){
  ...
  Wholdout.gibbs[k, 1, ] <- W[[1]][2, holdout.vector]
  ...
  for (g in 2:G){
    ...
    Wholdout.gibbs[k, g, ] <- W[[1]][2, holdout.vector]
  }
}
t2 <- Sys.time()
t2 - t1
...
cbind(Wholdout.gibbs[1, 1, ], input.data$Y[[1]][2, holdout.vector])

burn <- 1:50

Yholdout.gibbs <- pmax(Wholdout.gibbs, 0)

Y.hat <- apply(Yholdout.gibbs[, -burn, ], 3, mean)
Y.obs <- input.data$Y[[1]][2, holdout.vector]

plot(Y.obs, Y.hat)
sd(Y.obs - Y.hat, na.rm = TRUE)
