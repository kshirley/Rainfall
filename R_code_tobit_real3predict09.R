...
###subset until 2008
#data$date.string <- as.Date(date.string, "%Y/%m/%d")
#test<-subset(data, as.Date(date.string)< as.Date("2009-01-01"), with=FALSE)
date.string<-date.string[1:which(date.string=='2008-12-31')]
data<-data[,1:which(date.string=='2008-12-31')]
T<-dim(data)[2]
...
nino <- nino[1:which(nino.dates=="2008 December")]; nino.dates <- nino.dates[1:which(nino.dates=="2008 December")]  # nino only goes through April 2010.

###subset until 1992 onwards for nino 
# subselect just the months that align with the rainfall data:
#what index should the data start at?
w <- which(nino.dates=="1991 October") # start in oct so it has a 3 month lag match with the actual data
nino <- nino[w:which(nino.dates=="2008 December")]
nino.dates <- nino.dates[w:which(nino.dates=="2008 December")]
...
# OK, now nino is length 204, starting in January 1992 (3 months before rainfall data starts) 
#and ending in December 2008
...
#subset and repeat dates so nino aligns with month.day, which aligns with original data
####KV: IS THIS WRONG??? month.days starts in January, but nino starts in October
#No, Kenny says there's a 3 month lag in el nino's effect
month.mat[cbind(1:T,month.vec)] <- rep(nino[1:204],c(rep(month.days,17)))
#,month.days[1:3]
...
#subset..T should not be harcoded
T <- dim(data)[2]
S <- 6
J <- c(2,2,2,2,2,5)
P <- 5
...
# KV - for each column (10 all together)create a sine 
# and a cosine
...
#month. mat is a month indicator matrix
...
#Not sure I understand this re-starting values KV
...
### 1.d Set Priors
##################
#I think this is the prior on the Var Cov Matrix
v.0 <- J
...
gibbs.list <- list(mu.gibbs=mu.gibbs,sigma.gibbs=sigma.gibbs,alpha.gibbs=alpha.gibbs,lambda.gibbs=lambda.gibbs,tau.gibbs=tau.gibbs,beta.gibbs=beta.gibbs,
                   mu.arc.gibbs=mu.arc.gibbs,tau.arc.gibbs=tau.arc.gibbs,beta.arc.gibbs=beta.arc.gibbs,Sigma.gibbs=Sigma.gibbs,W.gibbs=W.gibbs, W=W)

save(gibbs.list,file=paste(path,"gibbs_09out_09232014_G5000.RData",sep=""))
...
#The point is that the all the posterior distributions now were generated without using 2010. 
#Everything can continue as business as usual form here on. But the key is that you load
#the parameter estimates from the 92-08 run
...
save(start.list,file=paste(path,"start_list09_v10.RData",sep=""))
...
g.vec <- round(seq(3000, 5000, length=50))
...
save(post.list,file=paste(path,"post_list_San095000_092314.RData",sep=""))
load(file=paste(path,"post_list_San095000_092314.RData",sep=""))
...
save.image("~/SkyDrive/IRI/RainfallSimulation/R/Rainfall/post_sim_output092314.RData")
...
#this pulls out the consec dry days across 6205 days per site and histograms into 100 bins
